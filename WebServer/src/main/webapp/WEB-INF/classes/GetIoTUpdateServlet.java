...
@WebServlet("/get_iot_updates")
public class GetIoTUpdateServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        if(map.isEmpty()){
            try {
                req.getRequestDispatcher("/pages/GetIoTUpdatesPage.html").include(req, resp);
    ...
    private void getRequestWithParam(HttpServletRequest req, HttpServletResponse resp, Map<String, String[]> map) throws IOException {
        ...
        int iotID = Integer.parseInt(map.get("iot_id")[0]);
        ...
        if(jsonObject.get("status").getAsInt() == 200){
            try {
                if(jsonObject.get("data").getAsJsonObject().isEmpty()){
                    HandleRespond.showError(req, resp, "{'title': 'Product not found'}");
                }
                else{
                    JsonObject js = handleRequestToGatewayServer(companyID, productID, iotID);
                ...
                try {
                    HandleRespond.showError(req, resp, "Fail to get Iot updates");
    ...
    private JsonObject handleRequestToGatewayServer(int companyID, int productID, int iotID) throws ServerDownException {

        String targetURL = "http://127.0.0.1:4000/company/" + companyID + "/product/" + productID + "/iot/" + iotID + "/update";
        ...
        try(CloseableHttpClient httpclient = HttpClients.createDefault()){
            ...
            try(CloseableHttpResponse response = httpclient.execute(request)){
                ...
                throw new RuntimeException("Fail to get Iot updates");
